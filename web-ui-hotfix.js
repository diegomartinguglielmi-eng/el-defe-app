// El Defe · UX hotfix 2026-09-12 v3: Competencias estable + torneos + ingreso real
(() => {
  const API='https://el-defe-v5-production.up.railway.app';
  const MARK='DEFE_UI_HOTFIX_20260912_V3';
  ...
  function makeCompVisual(btn,logo){
    if(!btn||btn.dataset.defeVisual==='1')return;
    const comp=btn.dataset.comp;
    btn.dataset.defeVisual='1';
    btn.style.minHeight='92px';btn.style.padding='8px 4px';btn.style.borderRadius='16px';btn.style.display='flex';btn.style.flexDirection='column';btn.style.alignItems='center';btn.style.justifyContent='center';btn.style.gap='5px';btn.style.fontSize='10px';btn.style.overflow='hidden';
  ...
  function chooseDivision(kind){const sel=document.getElementById('matchDivisionFilter'),opt=findDivisionOption(kind);if(!sel||!opt||sel.value===opt.value)return;sel.value=opt.value;sel.dispatchEvent(new Event('change',{bubbles:true}));}

  function fefiMode(){
    const fefi=document.querySelector('#matchFilters [data-comp="FEFI"]');if(!active(fefi))return;
    const card=fefi.closest('.card');if(!card||card.querySelector('[data-fefi-mode]'))return;
    const topGrid=fefi.parentElement,mode=document.createElement('div');mode.dataset.fefiMode='1';mode.style.cssText='display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:10px';
    ...
    const babyActive=current==='ALL'||(babyOpt&&current===babyOpt.value),futsalActive=futsalOpt&&current===futsalOpt.value;
    ...
    mode.querySelector('[data-fefi-kind="baby"]').onclick=()=>chooseDivision('baby');mode.querySelector('[data-fefi-kind="futsal"]').onclick=()=>chooseDivision('futsal');
  ...
  function fixLeagueTabs(){
    ...
    const current=[...grid.querySelectorAll(':scope > [data-comp]')].map(b=>b.dataset.comp).join('|'),desired=buttons.map(b=>b.dataset.comp).join('|');
    if(current!==desired)buttons.forEach(b=>grid.appendChild(b));
