// El Defe · UX hotfix 2026-09-12: ligas sin "Todos" + ingreso real
(() => {
  const API='https://el-defe-v5-production.up.railway.app';
  const MARK='DEFE_UI_HOTFIX_20260912';

  function fixLeagueTabs(){
    const allBtn=document.querySelector('#matchFilters [data-comp="ALL"]');
    if(allBtn){
      const grid=allBtn.parentElement;
      // Si la agenda arrancó en ALL, seleccionamos FEFI antes de retirar el botón global.
      const active=allBtn.classList.contains('btn');
      if(active){
        const fefi=grid?.querySelector('[data-comp="FEFI"]');
        if(fefi) setTimeout(()=>fefi.click(),0);
      }
      allBtn.remove();
      if(grid){
        grid.style.gridTemplateColumns='repeat(4,minmax(0,1fr))';
        grid.style.gap='7px';
        grid.querySelectorAll('[data-comp]').forEach(b=>{
  ...
  function tokenFromStorage(){
    return localStorage.getItem('defe_auth_token')||sessionStorage.getItem('defe_auth_token')||'';
  }
  ...
  function openLogin(){
    ...
    modal.style.cssText='position:fixed;inset:0;z-index:100000;background:rgba(8,28,58,.56);display:flex;align-items:center;justify-content:center;padding:20px';
    modal.innerHTML=`<div style="width:min(420px,100%);background:#fff;border-radius:24px;padding:24px;box-shadow:0 24px 60px #0004;color:#17365f">
      <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;margin-bottom:6px">
        <div><div style="font-size:12px;font-weight:900;letter-spacing:.08em;color:#0b4a8f">EL DEFE</div><h2 style="margin:4px 0 0;font-size:27px">Ingresar</h2></div>
      ...
      <p style="margin:0 0 18px;color:#718096;font-size:14px">Accedé con tu usuario del Club.</p>
      <form data-form>
    ...
    form.onsubmit=async e=>{
      e.preventDefault();
      msg.textContent='';submit.disabled=true;submit.textContent='Ingresando…';
      try{
        const fd=new FormData(form);
        const body=new URLSearchParams();body.set('username',String(fd.get('username')||'').trim());body.set('password',String(fd.get('password')||''));
