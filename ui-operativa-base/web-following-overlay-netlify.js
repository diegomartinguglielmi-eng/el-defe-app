(() => {
  ...
  css.textContent=`
    ...
    #defe-following-home{margin:14px 0;background:#fff;border:1px solid #dbe4ef;border-radius:18px;padding:14px;color:#112f55;box-shadow:0 2px 8px #0000000d}
    ...
    @media(max-width:640px){.df-event{padding:10px}.df-event-rival{font-size:14px}}
  `;
  ...
  function selectionCard(){
    ...
    return `<section id="defe-following-home"><div class="df-head"><div><div class="df-title">Qué quiero seguir</div><div class="df-sub">Elegí categorías de una o varias ligas.</div></div><button type="button" class="df-edit" data-follow-edit>${state.editing?'Cerrar':'Editar'}</button></div>${count?`<div class="df-chips">${chips()}</div>`:`<div class="df-empty" style="margin-top:10px">Todavía no elegiste equipos o categorías.</div>`}${editorHtml()}</section>`;
  }
  ...
  function agendaHtml(){
    ...
    return `<section id="defe-next-followed"><div class="df-agenda-head"><div class="df-agenda-title">Próximas fechas</div><div class="df-agenda-count">${state.events.length} próxima${state.events.length===1?'':'s'}</div></div>${shown.length?shown.map(eventHtml).join(''):`<div class="df-empty">Todavía no hay próximas fechas publicadas para tus selecciones.</div>`}${state.events.length>3?`<button type="button" class="df-edit" data-agenda-more style="width:100%;margin-top:2px">Ver agenda completa</button>`:''}</section>`;
  }

  function hideLegacyNext(){
    const heading=[...document.querySelectorAll('h1,h2,h3,div')].find(el=>el.childElementCount===0&&/^Próxima fecha$/i.test((el.textContent||'').trim())&&!el.closest('#defe-next-followed'));
    if(!heading)return;
    let n=heading;
  ...
  function renderHome(){
    ...
    if(!select){const wrap=document.createElement('div');wrap.innerHTML=selectionCard();select=wrap.firstElementChild;home.prepend(select)}else select.outerHTML=selectionCard();
    ...
    agenda=document.getElementById('defe-next-followed');agenda.querySelector('[data-agenda-more]')?.addEventListener('click',showAgenda);
  }
  ...
  setInterval(()=>{if(state.ready){if(isHome()&&!document.getElementById('defe-following-home'))renderAll()}else if(token())load()},1200);
