(() => {
  ...
  const roleLabel=r=>r==='admin'?'Administrador':r==='profe'?'Profe':r==='tienda'?'Tienda':'Socio';
  ...
  function syncProfeAccount(){
    const t=getToken(),storedRole=String(localStorage.getItem('defe_role')||'').toLowerCase();if(!t||(storedRole||tokenRole(t))!=='profe')return;
