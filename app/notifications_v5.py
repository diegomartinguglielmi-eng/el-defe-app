...
def deliver_pushes(db, event):
    ...
    if not private_key or not public_key:
        return {"sent":0,"failed":0,"disabled":0,"eligible":0,"configured":False,"error":"VAPID not configured"}
    rows=db.query(PushSubscription).filter(PushSubscription.enabled==True).all()
    sent=disabled=failed=eligible=0; first_error=None
    for sub in rows:
        if not _wants(sub,event): continue
        eligible+=1
        try:
            webpush(subscription_info={"endpoint":sub.endpoint,"keys":{"p256dh":sub.p256dh,"auth":sub.auth}},data=_push_payload(event),vapid_private_key=private_key,vapid_claims={"sub":_vapid_subject()},ttl=60 if event.urgent else 3600)
            sent+=1
        except WebPushException as exc:
            failed+=1; status=getattr(getattr(exc,"response",None),"status_code",None)
            first_error=first_error or f"WebPush {status or 'error'}: {str(exc)[:180]}"
            print({"webpush":"failed","status":status,"detail":str(exc)[:300]})
            if status in (404,410): sub.enabled=False; disabled+=1
        except Exception as exc:
            failed+=1; first_error=first_error or f"{type(exc).__name__}: {str(exc)[:180]}"
            print({"webpush":"failed","type":type(exc).__name__,"detail":str(exc)[:300]})
...
def publish_event(db,*,event_type,title,body,competition=None,category=None,match_id=None,urgent=False):
    event=NotificationEvent(event_type=event_type,title=title[:180],body=body,competition=competition,category=category,match_id=match_id,urgent=urgent)
