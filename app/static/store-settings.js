...
(function(){
  ...
  async function ensureAdminSetting(){
    ...
    const card=document.getElementById('storeAdminCard');if(!card||document.getElementById('storeWhatsappSetting'))return;
    let current='';try{current=(await api('/api/store/admin/settings')).whatsapp_number||'';}catch{return;}
  ...
  function init(){const mo=new MutationObserver(()=>ensureAdminSetting());mo.observe(document.body,{childList:true,subtree:true});setTimeout(ensureAdminSetting,600);}
