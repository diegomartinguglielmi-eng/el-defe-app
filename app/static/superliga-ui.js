...
(function(){
  ...
  function renameLabels(){
    document.querySelectorAll('.defe-topbar-title').forEach(el=>{if(String(el.textContent||'').trim().toUpperCase()==='PARTIDOS')el.textContent='COMPETENCIAS';});
    ...
    document.querySelectorAll('[aria-label="Partidos"],[title="Partidos"]').forEach(el=>{if(el.getAttribute('aria-label')==='Partidos')el.setAttribute('aria-label','Competencias');if(el.getAttribute('title')==='Partidos')el.setAttribute('title','Competencias');});
  }
  function partidosScreen(){
    const title=[...document.querySelectorAll('.defe-topbar-title')].find(el=>{const t=String(el.textContent||'').trim().toUpperCase();return visible(el)&&(t==='PARTIDOS'||t==='COMPETENCIAS');});
  ...
  async function mount(){
    ...
    content.innerHTML='<div id="allMatches"><div class="card">Cargando competencias…</div></div>';
    try{await window.defeLoadMatchExplorer();}
    catch(err){const t=content.querySelector('#allMatches');if(t)t.innerHTML='<div class="card"><b>No se pudo cargar la información de competencias</b><div class="meta">'+String(err?.message||err)+'</div></div>';}
  }
  function schedule(){renameLabels();clearTimeout(timer);timer=setTimeout(mount,120);}
  function run(){installStyles();renameLabels();document.addEventListener('click',schedule,true);observer=new MutationObserver(schedule);observer.observe(document.getElementById('app')||document.body,{subtree:true,childList:true});window.addEventListener('popstate',schedule);window.addEventListener('hashchange',schedule);schedule();}
