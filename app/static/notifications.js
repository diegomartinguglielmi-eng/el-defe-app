(function(){
  ...
  async function poll(){
    try{
      ...
      if(!initialized){localStorage.setItem(LAST,String(max));initialized=true;return;}
  ...
  function ensurePermissionCard(){
    ...
    card.innerHTML=`<div class="row"><b>Avisos de El Defe</b><span id="notificationState" class="badge ${granted?'ok':''}">${granted?'ACTIVOS':'DESACTIVADOS'}</span></div><div class="meta">Recibí urgentes y cambios de horario de las categorías que seguís en este dispositivo.</div><button id="notificationEnableBtn" class="btn" style="margin-top:10px">${granted?'Avisos habilitados':'Activar avisos'}</button><div id="notificationInbox" class="meta"></div>`;
