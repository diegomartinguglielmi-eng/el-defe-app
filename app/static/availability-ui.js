...
(function(){
  ...
  function token(){for(const st of [localStorage,sessionStorage])for(let i=0;i<st.length;i++){const t=jwtFromValue(st.getItem(st.key(i)));if(t)return t}return localStorage.getItem('defe_token')||localStorage.getItem('defe_auth_token')||''}
  function role(){return String(localStorage.getItem('defe_role')||'').toLowerCase()}
  ...
  function ensureAdminEntry(){
    ...
    const main=document.querySelector('#admin .main')||document.querySelector('#admin');
    if(!main||main.querySelector('[data-av-admin-entry]'))return;
    const c=document.createElement('div');c.className='av-admin-entry';c.dataset.avAdminEntry='1';c.innerHTML='<b>Confirmaciones de asistencia</b><div style="font-size:12px;color:#718096;margin:4px 0 10px">Seguimiento por liga, categoría y próxima fecha.</div><button>Abrir tablero de asistencia</button>';main.prepend(c);c.querySelector('button').onclick=openAdmin;
  }
