...
(function(){
  ...
  function ensureAdminFixed(){
    const allowed=['admin','delegado','dt'].includes(role())&&!!token();
    const current=document.querySelector('[data-av-admin-fixed]');
    if(!allowed){current?.remove();return}
    if(current)return;
    const b=document.createElement('button');b.className='av-admin-fixed';b.dataset.avAdminFixed='1';b.innerHTML='<span>✓</span> Asistencia';b.onclick=openAdmin;document.body.appendChild(b);
  }
  ...
  function tick(){injectUser();ensureAdminEntry();ensureAdminFixed()}
