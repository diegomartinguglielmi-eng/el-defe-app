...
class Standing(Base):
    ...
    gc: Mapped[int|None]=mapped_column(Integer)
    gd: Mapped[int|None]=mapped_column(Integer)
    source_url: Mapped[str|None]=mapped_column(Text)
    updated_at: Mapped[str]=mapped_column(DateTime(timezone=True),server_default=func.now(),onupdate=func.now())
...
class CallUpPlayer(Base):
    __tablename__="callup_players"
    id: Mapped[int]=mapped_column(primary_key=True)
    callup_id: Mapped[int]=mapped_column(ForeignKey("callups.id"),index=True)
    person_id: Mapped[int]=mapped_column(ForeignKey("people.id"),index=True)
    attendance: Mapped[str]=mapped_column(String(30),default="pending")
    starter: Mapped[bool]=mapped_column(Boolean,default=False)
    minutes: Mapped[int|None]=mapped_column(Integer)
    __table_args__=(UniqueConstraint("callup_id","person_id",name="uq_callup_person"),)
...
class PlayerMatchStat(Base):
    ...
    minutes: Mapped[int|None]=mapped_column(Integer)
    __table_args__=(UniqueConstraint("match_id","person_id",name="uq_match_person_stat"),)
...
class NotificationPreference(Base):
    ...
    match_changes: Mapped[bool]=mapped_column(Boolean,default=True)
    results: Mapped[bool]=mapped_column(Boolean,default=True)
    news: Mapped[bool]=mapped_column(Boolean,default=True)
    callups: Mapped[bool]=mapped_column(Boolean,default=True)
    favorite_only: Mapped[bool]=mapped_column(Boolean,default=False)
...
class MediaItem(Base):
    ...
    media_type: Mapped[str]=mapped_column(String(30),default="photo")
