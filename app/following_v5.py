...
def _clean_key(value: str) -> str | None:
    ...
    if "|" not in raw:
        return None
    competition, category = raw.split("|", 1)
    competition = competition.strip().upper()
    category = category.strip()
    if competition not in COMPETITIONS or not category:
        return None
...
def _migrate_legacy(db: Session, user_id: int) -> int:
    created = 0
    rows = db.query(Favorite).filter(Favorite.user_id == user_id, Favorite.favorite_type.in_(LEGACY_TYPES)).all()
    for row in rows:
        ...
        if category in FEFI_BABY_DEFAULT:
            ...
            exists = db.query(Favorite).filter(
                ...
                Favorite.favorite_id == key,
            ).first()
            if not exists:
                db.add(Favorite(user_id=user_id, favorite_type=FOLLOW_TYPE, favorite_id=key))
                created += 1
    if created:
        db.commit()
    return created
...
def _options(db: Session) -> dict[str, list[str]]:
    ...
    for (category,) in db.query(FefiCategorySchedule.category).distinct().all():
        if category:
            result["FEFI"].add(str(category).strip())

    for team in db.query(Team).filter(Team.is_active == True).all():
        comp = (team.competition or "").strip().upper()
        div = (team.division or "").strip()
        if comp in result and div and not (comp == "FEFI" and div.lower() == "zona h"):
            result[comp].add(div)
    ...
    def sort_key(v: str):
        if v.isdigit():
            return (0, int(v))
        low = v.lower()
        order = {"1ra": 1, "2da": 2, "3ra": 3, "4ta": 4, "5ta": 5, "6ta": 6, "7ma": 7, "8va": 8}
...
def _date_parts(value: str | None) -> tuple[str | None, str | None]:
    if not value:
        return None, None
    raw = str(value).strip()
    date = raw[:10] if len(raw) >= 10 else raw
    time = None
    if "T" in raw and len(raw.split("T", 1)[1]) >= 5:
        time = raw.split("T", 1)[1][:5]
    elif " " in raw:
        tail = raw.split(" ", 1)[1]
        if len(tail) >= 5 and tail[2:3] == ":":
            time = tail[:5]
    return date, time
...
def _is_defe(name: str | None) -> bool:
    low = (name or "").lower()
    return "defensores" in low or low.strip() in {"defe", "def. de santos lugares", "defensores de sl"}
...
def _event_from_match(db: Session, m: Match, competition: str, category: str) -> dict:
    date, time = _date_parts(m.date)
    note = None
    if competition == "FEFI" and category.isdigit():
        sched = db.query(FefiCategorySchedule).filter(
            FefiCategorySchedule.match_id == m.id,
            FefiCategorySchedule.category == category,
        ).first()
        if sched:
            time = sched.time or time
            note = sched.note

    local = _is_defe(m.home)
    club = m.home or None
    venue = (m.venue or "").strip() or None
    address = venue
    if not address and competition == "FEFI" and local:
        address = "Ernesto Sábato 3162, Santos Lugares, Buenos Aires"
    map_query = address or venue or club
...
def _events_for_selection(db: Session, selection: str, today: str) -> list[dict]:
    ...
    rows = _canonical(db.query(Match).filter(Match.competition == competition).all())
    candidates = []
    for m in rows:
        ...
        if m.home_score is not None and m.away_score is not None:
            continue
        if competition == "FEFI" and category.isdigit():
            if (m.division or "").lower() not in {"zona h", "h"}:
                continue
        elif (m.division or "").strip().lower() != category.strip().lower():
            continue
        candidates.append(_event_from_match(db, m, competition, category))
    candidates.sort(key=lambda x: ((x["date"] or "9999-99-99"), (x["time"] or "99:99"), x["match_id"]))
    return candidates
...
def following_options(db: Session = Depends(get_db)):
    ...
    return {
        "competitions": [
            {"competition": comp, "categories": grouped.get(comp, [])}
...
def get_following(db: Session = Depends(get_db), user=Depends(get_current_user)):
    selections = _current(db, user.id)
    return {"selections": selections, "count": len(selections)}
...
def set_following(payload: FollowingIn, db: Session = Depends(get_db), user=Depends(get_current_user)):
    ...
    for key in clean:
        db.add(Favorite(user_id=user.id, favorite_type=FOLLOW_TYPE, favorite_id=key))
    db.commit()
    return {"ok": True, "selections": clean, "count": len(clean)}
