...
def _nearby_tournament_marker(table) -> str | None:
    # Search backwards for the closest compact DOM label that says Apertura/Clausura.
...
def _prefer_clausura(candidates: list) -> list:
    ...
    if clausura:
        return clausura
    # Fallback for the current official page order (Apertura first, Clausura later):
    # use only the last matching tournament table instead of merging both.
...
def parse_fefi(html: str) -> dict[str, Any]:
        ...
        if "DIRECCIÓN" in text or "DIRECCION" in text:
            for row in table.find_all("tr"):
                ...
                if len(cs) >= 3 and _norm(cs[0]) not in {"CLUB","EQUIPO","NOMBRE"}:
                    name = _norm(cs[0])
                    if name:
                        venues[name] = {"name":name,"address":cs[1].strip(),"locality":cs[2].strip()}

    fixture_candidates = []
    for table in tables:
        text = _norm(table.get_text(" ", strip=True))
        if "LOCAL" in text and "VISITANTE" in text and " VS " in f" {text} ":
            fixture_candidates.append(table)
    ...
    for table in _prefer_clausura(fixture_candidates):
        ...
        for row in table.find_all("tr"):
            ...
            if not cs: continue
            joined = " ".join(cs)
            if re.search(r"Fecha\s+\d+\s*-", joined, re.I):
                current_round, current_date = _round_date(joined)
                continue
            if current_round and len(cs) >= 3 and _norm(cs[1]) == "VS":
                ...
                if FEFI_CLUB in (home, away):
                    ...
                    fixtures.append({
                        "round":current_round,
                        "round_name":f"Fecha {current_round}",
                        "date":current_date,
                        "home":home,
                        "away":away,
                        "venue":None if not venue else f"{venue['name']} · {venue['address']} · {venue['locality']}"
                    })

    for item in fixtures:
        venue = venues.get(item["home"])
        if venue:
            item["venue"] = f"{venue['name']} · {venue['address']} · {venue['locality']}"
...
def run_fefi_pending_sync(db: Session | None = None):
    ...
    try:
        ...
        db.add(FefiRawSnapshot(sha256=sha,html=html)); db.commit()
        ...
        for item in parsed["fixtures"]:
            ...
            before = _match_payload(current)
            proposed = {"round_name":item["round_name"],"date":item["date"],"home":item["home"],"away":item["away"],"venue":item["venue"],"status":"scheduled"}
...
def _apply_change(db: Session, change: FefiPendingChange):
    ...
    if not match:
        match = Match(external_key=f"FEFI|2026|H|CLAUSURA|{payload['round_name']}",competition="FEFI",division=FEFI_DIVISION,source_url=FEFI_URL,source_kind="approved_sync",home=payload["home"],away=payload["away"]); db.add(match)
...
def pending_changes(db:Session=Depends(get_db),user=Depends(require_roles("admin","delegado"))):
    rows=db.query(FefiPendingChange).filter(FefiPendingChange.status=="pending").order_by(FefiPendingChange.detected_at.desc()).all()
    return [{"id":r.id,"type":r.change_type,"detail":r.detail,"before":json.loads(r.before_json) if r.before_json else None,"after":json.loads(r.after_json),"status":r.status,"detected_at":r.detected_at} for r in rows]

@router.get("/status")
def fefi_status(db:Session=Depends(get_db),user=Depends(require_roles("admin","delegado"))):
    latest=db.query(SyncRun).filter(SyncRun.source=="FEFI_PENDING").order_by(SyncRun.id.desc()).first(); pending=db.query(FefiPendingChange).filter(FefiPendingChange.status=="pending").count()
    return {"source":"FEFI 2026 · Zona H · Clausura","pending":pending,"last_run":None if not latest else {"status":latest.status,"detail":latest.detail,"created_at":latest.created_at},"schedule":"cada 4 horas"}
...
def approve_change(change_id:int,db:Session=Depends(get_db),user=Depends(require_roles("admin","delegado"))):
    ...
    match=_apply_change(db,change); change.status="approved"; change.resolved_at=datetime.now(timezone.utc); db.commit(); db.refresh(match)
