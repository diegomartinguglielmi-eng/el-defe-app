...
def parse_fefi(html: str) -> dict[str, Any]:
        ...
        if "LOCAL" in text and "VISITANTE" in text and " VS " in f" {text} ":
            ...
            for row in table.find_all("tr"):
                ...
                if re.search(r"Fecha\s+\d+\s*-", joined, re.I):
                    current_round, current_date = _round_date(joined); continue
    ...
    for item in fixtures:
        ...
        if venue: item["venue"] = f"{venue['name']} · {venue['address']} · {venue['locality']}"
    unique = {item["round"]: item for item in fixtures}
    return {"fixtures":[unique[k] for k in sorted(unique)],"venues":venues}
...
def _pending_key(round_number: int, payload: dict[str, Any]) -> str:
    ...
    return f"FEFI|2026|H|F{round_number}|{digest}"
...
def run_fefi_pending_sync(db: Session | None = None):
    ...
    try:
        ...
        db.add(SyncRun(source="FEFI_PENDING",status="ok",detail=f"{created} cambios nuevos; {unchanged} sin cambios")); db.commit()
        return {"ok":True,"fixtures":len(parsed["fixtures"]),"changes_created":created,"unchanged":unchanged,"snapshot_sha256":sha}
...
def _apply_change(db: Session, change: FefiPendingChange):
    ...
    if not match:
        match = Match(external_key=f"FEFI|2026|H|{payload['round_name']}",competition="FEFI",division=FEFI_DIVISION,source_url=FEFI_URL,source_kind="approved_sync",home=payload["home"],away=payload["away"]); db.add(match)
...
def fefi_status(db:Session=Depends(get_db),user=Depends(require_roles("admin","delegado"))):
    ...
    return {"source":"FEFI 2026 · Zona H","pending":pending,"last_run":None if not latest else {"status":latest.status,"detail":latest.detail,"created_at":latest.created_at},"schedule":"cada 4 horas"}
