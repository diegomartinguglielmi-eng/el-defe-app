(() => {
 ...
 function apply(u){const raw=String(u?.role||'guest').toLowerCase();const role=(raw==='coach'||raw==='profesor'||raw==='entrenador'||raw==='dt')?'profe':raw;document.documentElement.dataset.defeRole=role;document.querySelectorAll('[data-role-only]').forEach(el=>{const allowed=(el.dataset.roleOnly||'').split(',').map(x=>x.trim());el.hidden=!allowed.includes(role)});window.dispatchEvent(new CustomEvent('defe:role',{detail:{role,user:u}}));setTimeout(paint,100);setTimeout(paint,800);if(role==='profe')setTimeout(loadAdminData,250)}
