(() => {
 ...
 function renderProfeDashboard(){
   ...
   const renderKey=(adminData?'data':'empty')+'-v2';if(d.dataset.rendered===renderKey)return;d.dataset.rendered=renderKey;
   ...
   d.querySelectorAll('button[data-profe-dash-plant],button[data-profe-dash-com]').forEach(x=>x.style.cssText+='border:1px solid #dbe4ef;border-radius:16px;padding:15px 4px;background:#fff;color:#17365f;font-weight:850');
   const fixture=d.querySelector('[data-profe-dash-fixture]');if(fixture)fixture.onclick=openProfeFixture;
   d.querySelector('[data-profe-dash-att-main]').onclick=()=>window.DefeProfe?.openAttendance?.();d.querySelector('[data-profe-dash-com]').onclick=()=>window.DefeProfe?.openCommunications?.();
