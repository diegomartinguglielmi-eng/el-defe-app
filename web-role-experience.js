(() => {
 ...
 function renderProfeDashboard(){
   ...
   const renderKey=adminData?'data':'empty';if(d.dataset.rendered===renderKey)return;d.dataset.rendered=renderKey;
   d.innerHTML='<section><div style="display:flex;align-items:center;gap:9px"><div style="font-size:26px;font-weight:950;color:#17365f">Hola, Profe</div><span style="font-size:12px;background:#e5f0fb;color:#15589e;padding:6px 10px;border-radius:999px;font-weight:900">PROFE</span></div><div style="color:#64748b;margin-top:5px">Gestión deportiva de tus equipos</div></section><section style="background:#fff;border:1px solid #dbe4ef;border-radius:20px;padding:18px"><div style="display:flex;justify-content:space-between;gap:10px"><b>Próxima jornada</b><button data-profe-dash-fixture style="border:0;background:none;color:#15589e;font-weight:800">Ver fixture ›</button></div><div style="font-weight:900;margin-top:14px">'+(parts.filter(Boolean).join(' · ')||'Plantel')+'</div><div style="font-size:21px;font-weight:950;margin-top:7px">'+(item.rival?'El Defe vs '+item.rival:'Partido a confirmar')+'</div><div style="color:#64748b;margin-top:7px">'+fmtDate(item.date)+(item.time?' · '+item.time:'')+(item.venue?' · '+item.venue:'')+'</div></section><section style="background:#fff;border:1px solid #dbe4ef;border-radius:20px;padding:18px"><div style="display:flex;justify-content:space-between"><b>Asistencia</b><button data-profe-dash-att style="border:0;background:none;color:#15589e;font-weight:900">Ver detalle ›</button></div><div style="display:grid;grid-template-columns:repeat(4,1fr);gap:7px;margin-top:16px;text-align:center"><div><b style="font-size:22px">'+(item.yes||0)+'</b><small style="display:block">Confirmados</small></div><div><b style="font-size:22px">'+(item.no||0)+'</b><small style="display:block">No asisten</small></div><div><b style="font-size:22px">'+((item.maybe||0)+(item.pending||0))+'</b><small style="display:block">Pendientes</small></div><div><b style="font-size:22px">'+total+'</b><small style="display:block">Jugadores</small></div></div><button data-profe-dash-att-main style="margin-top:16px;width:100%;border:0;border-radius:13px;padding:13px;background:#15589e;color:#fff;font-weight:900">Ver asistencia de la jornada →</button></section><div style="font-weight:950;font-size:19px;color:#17365f">Herramientas del Profe</div><section style="display:grid;grid-template-columns:repeat(3,1fr);gap:9px"><button data-profe-dash-att>✓<br>Asistencia<br>Jornada</button><button data-profe-dash-plant>👥<br>Plantel<br>Jugadores</button><button data-profe-dash-com>✉<br>Comunicaciones</button></section>';
   d.querySelectorAll('button[data-profe-dash-att],button[data-profe-dash-plant],button[data-profe-dash-com]').forEach(x=>x.style.cssText+='border:1px solid #dbe4ef;border-radius:16px;padding:15px 4px;background:#fff;color:#17365f;font-weight:850');
   const fixture=d.querySelector('[data-profe-dash-fixture]');if(fixture)fixture.onclick=()=>{const close=page.querySelector('[data-md-close]');close?.click();setTimeout(()=>{const x=[...document.querySelectorAll('button,a')].find(x=>/fixture/i.test((x.textContent||'').trim())&&x.offsetParent!==null);x?.click()},0)};
   d.querySelectorAll('[data-profe-dash-att]').forEach(x=>x.onclick=()=>window.DefeProfe?.openAttendance?.());d.querySelector('[data-profe-dash-att-main]').onclick=()=>window.DefeProfe?.openAttendance?.();d.querySelector('[data-profe-dash-com]').onclick=()=>window.DefeProfe?.openCommunications?.();
   const plant=d.querySelector('[data-profe-dash-plant]');if(plant)plant.onclick=()=>{const x=[...document.querySelectorAll('button,a')].find(x=>/Plantel \/ Jugadores|Planteles/i.test((x.textContent||'').trim())&&!x.closest('[data-profe-dashboard]')&&x.offsetParent!==null);x?.click()};
 }
 function adminHome(){
