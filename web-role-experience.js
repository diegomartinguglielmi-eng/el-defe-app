(() => {
 ...
 function renderProfeDashboard(){
   ...
   if(!d.dataset.delegated){d.dataset.delegated='1';d.addEventListener('click',e=>{const b=e.target.closest('button');if(!b)return;if(b.matches('[data-profe-dash-fixture]')){e.preventDefault();e.stopImmediatePropagation();openProfeFixture()}else if(b.matches('[data-profe-dash-plant]')){e.preventDefault();e.stopImmediatePropagation();openPlantel()}else if(b.matches('[data-profe-dash-com]')){e.preventDefault();e.stopImmediatePropagation();window.DefeProfe?.openCommunications?.()}else if(b.matches('[data-profe-dash-att-main]')){e.preventDefault();e.stopImmediatePropagation();window.DefeProfe?.openAttendance?.()}},true)};
 }
 function adminHome(){
