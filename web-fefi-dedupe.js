// DEFE_FEFI_DEDUPE_20260912_V1
// El Defe · layout autoritativo de Competencias 2026-09-12
(() => {
  const MARK='DEFE_COMPETENCIAS_LAYOUT_V2';
  let timer=null;
  ...
  function statusBlock(l){
    ...
    return `<div class="defe-status" style="grid-template-columns:repeat(${Math.max(3,rows.length)},minmax(0,1fr))">${rows.map(b=>`<button type="button" data-ui-status="${b.dataset.status}" class="${cur===b.dataset.status?'on':''}">${labels[b.dataset.status]}</button>`).join('')}</div>`;
  }
  function ensureCss(){
...
.defe-cats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px}.defe-tours{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:10px}.defe-status{display:grid;gap:10px;margin-top:14px}.defe-status button{font-size:15px;min-height:52px}
@media(max-width:390px){#defe-competencias-ui{padding:14px}.defe-leagues{gap:7px}.defe-league{min-height:116px;padding:8px 4px}.defe-league img{width:56px;height:56px}.defe-cats{gap:6px}}
`;
  ...
  function render(){
    const l=findLegacy();if(!l||!l.comp.FEFI)return;
    ensureCss();
