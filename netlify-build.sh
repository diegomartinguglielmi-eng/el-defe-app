...
grep -F "availability-ui-${SHA}.js" defe-web-build/dist/index.html\ngrep -F "web-coach-nav-${SHA}.js" defe-web-build/dist/index.html
grep -F "DEFE_COMPETENCIAS_LAYOUT_V5" "defe-web-build/dist/${FEFIDEDUPE}"
set -e

echo "Netlify build listo: design system + Mi Defe + navegación + Competencias + Tienda + Mis pedidos + Panel Tienda + Disponibilidad"
